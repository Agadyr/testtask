...
export default function Order({item, index, deleteItem}){
    ...
    return (
        <div>
            <div className="row tp flex row-flex" key={index}>
                ...
                <div className="col">
                    ...
                    {(item.status == 'Отменен') && <span style={{color:"#FF316A"}}>&bull; <p>{item.status}</p></span>}
                </div>
            ...
            <div className="row tp flex row-none" key={index}>
                    <div className="col col_user">
                        <img src={item.contragent_img} alt="" />
                        <div style={{marginLeft:"10px"}}>
                            <h2>{item.contragent_full_name}</h2>
                    ...
                    <div className="col fd aife">
                        <p>{item.sum} ₸</p>
                        {(item.status == 'Новый' || item.status == 'Старый') && <p>{item.status}</p>}
                        {(item.status == 'Успешно') && <span style={{color:"#2BEFB3"}}><p>{item.status}</p></span>}
                        {(item.status == 'Отменен') && <span style={{color:"#FF316A"}}><p>{item.status}</p></span>}
                    </div>
                </div>
        </div>
    )
